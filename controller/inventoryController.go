...
func (ctrl *InventoryController) GetStockByOption(c *gin.Context) {
	...
	stock, err := ctrl.service.GetStockByOption(c.Request.Context(), id)
	if err != nil {
		SendError(c, "Get stock failed", err.Error(), http.StatusInternalServerError)
		return
	}
	SendResponse(c, true, "Get stock by option successfully", stock, http.StatusOK)
}
