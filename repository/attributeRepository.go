...
type AttributeRepository interface {
	...
	CreateValue(ctx context.Context, val *entity.AttributeValue) error
...
func (ar *attributeRepository) CreateValue(ctx context.Context, val *entity.AttributeValue) error {
	return ar.db.WithContext(ctx).Create(val).Error
}
func (ar *attributeRepository) ListAttributes(ctx context.Context) ([]entity.Attribute, error) {
	var attrs []entity.Attribute
	err := ar.db.WithContext(ctx).Preload("Values").Find(&attrs).Error
	if err != nil {
		return nil, err
	}
	return attrs, nil
